...
const Product = ({ product, isDark }) => {
  ...
  return (
    <Link
      to={`/product/${product}`}
      className="card w-auto bg-light hover:shadow-xl transition-all ease-in-out rounded-[10px] lg:rounded-[30px] overflow-hidden"
    >
      <figure className="relative">
        ...
        <span className="bg-secondary text-white lg:text-sm text-xs px-2 lg:px-5 py-1 rounded-r-[25px] absolute top-[30px] left-0 font-semibold">
        ...
        />
        <RoundedIcon
          className="absolute bg-gray-light top-[30px] right-5 cursor-pointer"
