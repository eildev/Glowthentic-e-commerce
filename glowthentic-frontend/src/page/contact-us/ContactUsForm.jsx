...
const ContactUsForm = () => {
    const [sendContactMessage, { isLoading, error, isSuccess }] = useSendContactMessageMutation();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error("Submission Error:", err?.data?.message || "Something went wrong");
        }
    };
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {["Branches", "Skin Type", "Special Offer And Promotion", "Returns And Refunds", "Product And Stock"].map((category) => (
                ...
                <button type="submit" className="block w-full lg:w-[128px] ml-auto px-[12px] py-[6px] lg:px-[16px] lg:py-[8px] text-sm lg:text-md font-normal font-encode border text-secondary border-secondary" disabled={isLoading}>
                    {isLoading ? "Sending..." : "Send"}
                </button>
                {isSuccess && <p style={{ color: "green" }}>Message sent successfully!</p>}
                {error && <p style={{ color: "red" }}>Failed to send message. Please try again.</p>}
            </form>
        </div>
    );
};
