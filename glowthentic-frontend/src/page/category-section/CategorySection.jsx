...
// import images from "../../assets/img/category/10.png";
    ...
    <div>
      ...
      <Container>
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-6">
          ...
          {data?.categories?.filter((category) => category?.parent_id === null).map((category, index) => (
              <div key={index} className="lg:px-16 px-5 md:px-10 items-center justify-center">
                ...
                />
                <h3 className="text-sm text-gray-600 mt-2 text-center">{category?.categoryName}</h3>
              </div>
            ))}
